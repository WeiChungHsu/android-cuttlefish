...
# Install kernel
apt-get install -y '^linux-image-6.6.*aosp15-linaro.*' '^linux-headers-6.6.*aosp15-linaro.*'
#apt install -y -t bookworm-backports linux-headers-arm64
#apt install -y -t bookworm-backports linux-image-arm64

# Only install AMD GPU driver, because Nvidia GPU driver has issue with aosp
...
# aosp kernel can't work with nvidia-container-toolkit
